...
public class PrintStatisticsDao extends GenericDaoImpl<PrintStatistics, Long>{
	private static final Logger log=LoggerFactory.getLogger(PrintStatistics.class);
	public PrintStatisticsDao(Class<PrintStatistics> type) {
		super(type);
	}
	// 选择工位类型下拉框数据
			public List getPrintStatisticssComboBox(final String whereSql) {
				...
				try {
					...
					log.error("getPrintStatisticssComboBox 出错：" + e.toString());
					e.printStackTrace();
				}
				return null;
			}
