...
import cn.com.topit.gl.service.VipRoomService;
public class VipRoomAction<VipRoom> extends GenericActionSupport {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(VipRoomAction.class);
	private VipRoomService service;
	private String uploadFileName;
	private File uploadFile;
	public void setService(VipRoomService service) {
	...
	public void saveImg() {
		...
		cn.com.topit.gl.dao.VipRoom vipRoom;
		...
		try {
			...
			vipRoom = service.saveByJSONObject(jsonObject);
			response.getWriter().print(
					"{success:true,topit_ext_id:" + vipRoom.getId() + "}");
	...
	// 根据查询条件获取记录集
			public String getVipRoomsComboBox() {
				try {
					records.put("records",  service
							.getVipRoomsComboBox(baseSql + whereSql+order));
				} catch (Exception e) {
					log.error("getVipRoomsComboBox出错："+e.toString());
					e.printStackTrace();
				}
				return "managerExt";
			}
