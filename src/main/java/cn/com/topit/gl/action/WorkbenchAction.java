...
public class WorkbenchAction<Workbench> extends GenericActionSupport {
	...
	public void setService(WorkbenchService service) {
	...
	// 根据查询条件获取记录集
			public String getWorkbenchsComboBox() {
				try {
					records.put("records",  service
							.getWorkbenchsComboBox(baseSql + whereSql+order));
				} catch (Exception e) {
					log.error("getWorkbenchsComboBox出错："+e.toString());
					e.printStackTrace();
				}
				return "managerExt";
			}
