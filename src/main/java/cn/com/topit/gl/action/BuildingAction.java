...
import cn.com.topit.gl.service.BuildingService;

public class BuildingAction<Building> extends GenericActionSupport {
	...
	public void save() {
		...
		cn.com.topit.gl.dao.Building building;
		...
		try {
			...
			building = service.saveByJSONObject(jsonObject);
			response.getWriter().print(
					"{success:true,topit_ext_id:" + building.getId() + "}");
			// System.out.println("topit_ext_id!" + building.getId());
		} catch (Exception e) {
			e.printStackTrace();
	...
	// 根据查询条件获取记录集
	public String getBuildingsComboBox() {
		try {
			records.put("records",
					service.getBuildingsComboBox(baseSql + whereSql + order));
		} catch (Exception e) {
