...
import cn.com.topit.gl.service.NewMeetingRoomService;
public class NewMeetingRoomAction<NewMeetingRoom> extends GenericActionSupport {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(NewMeetingRoomAction.class);
	private NewMeetingRoomService service;
	private String uploadFileName1;
	private File uploadFile1;
	private String uploadFileName2;
	private File uploadFile2;
	public void setService(NewMeetingRoomService service) {
		this.service = service;
		super.setAbstractService(service);
	}
	// 根据查询条件获取记录集
		public String getMeetingRoomComboBox() {
			try {
				records.put("records",  service
						.getMeetingRoomComboBox(baseSql + whereSql+order));
			} catch (Exception e) {
				log.error("getMeetingRoomComboBox出错："+e.toString());
				e.printStackTrace();
			}
			return "managerExt";
		}
	public void save() {
		...
		cn.com.topit.gl.dao.NewMeetingRoom meetingRoom;
