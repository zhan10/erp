...
<script>
	var mainFields = [
		...
		,{text : 'finishedStatus',dataIndex : 'finishedStatus',name : 'finishedStatus',width : 40,hidden : true}
		,{
	...
	var columnOperators = [{
		...
		items:[{
			...
			handler : function(grid, rowIndex, colIndex) {
				...
				var status = erpOrdersStatusDs.findRecord('text', '已入库').get('value')
				updateStatus("factory/erpOrders!save",rec,'成品入库','您是否确认此订单入库，请仔细核对订单！',status,"订单入库成功!")
				/*if(rec.get("status")==2){
					var status = erpOrdersStatusDs.findRecord('text', '财务审核').get('value')
					updateStatus("finished/erpOrders!save",rec,'发送财务审核','您是否确认发送此订单到财务审核，请仔细核对订单！',status,"订单发送成功!")
				}else{
					var status = erpOrdersStatusDs.findRecord('text', '财务二审').get('value')
					updateStatus("finished/erpOrders!save",rec,'发送财务二审','您是否确认发送此订单到财务二审，请仔细核对订单！',status,"订单发送成功!")
				}*/
			}
		}]
	}];
