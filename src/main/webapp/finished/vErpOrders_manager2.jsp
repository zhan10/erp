...
<head>
	...
	<%@ include file="finishedOrdersDefine.jsp"%>
	<%@ include file="finishedOrdersDataExt.jsp"%>
	<%@ include file="finishedOrdersFormExt.jsp"%>
	<%@ include file="finishedOrdersSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'finished/vErpOrders!managerExt', baseSql,order);
			ds.load();
					 ...
					 text: '删除',
						handler: function(widget, event) {   
							...
							if(ordersCode.substring(0, 1)=="N"){
								...
								updateStatus("finished/erpOrders!save",rec,'删除','您是否确认删除此订单，请仔细核对订单！',status,"订单删除成功!")
							}else{
								Ext.MessageBox.alert('错误', '无法删除该订单！');
							}
						//delFromDB(ds,sm,'finished/vErpOrders!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						/* <sec:authorize url="/finished/vErpOrders!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/finished/vErpOrders!cabinetAdd">,cabinetAddAction</sec:authorize>
						<sec:authorize url="/finished/vErpOrders!edit">,editAction</sec:authorize> 
						<sec:authorize url="/finished/vErpOrders!del">,delAction</sec:authorize> */
					 ]
				});
