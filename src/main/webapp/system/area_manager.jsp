...
<head>
	...
	<%@ include file="areaDefine.jsp"%>
	<%@ include file="areaDataExt.jsp"%>
	<%@ include file="areaFormExt.jsp"%>
	<%@ include file="areaSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'system/area!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('name', 100, '',0, '<s:text name="area.name" />'),
					'-',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/system/area!add">
						,getAddButton(win,winTitle + '——新增','新增'),
						getCopyAddButton(win,winTitle + '——拷贝添加', sm,'拷贝添加')
						</sec:authorize>
						<sec:authorize url="/system/area!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改'),
						getBatchButton(batchWin,sm,'批量修改')
						</sec:authorize>
						<sec:authorize url="/system/area!del">
						,getDelButton(ds, sm, 'system/area!delete','删除')
						</sec:authorize>
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'system/area!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/system/area!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/system/area!edit">,editAction</sec:authorize>
						<sec:authorize url="/system/area!del">,delAction</sec:authorize>
					 ]
				});
