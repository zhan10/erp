...
<head>
	...
	<%@ include file="vipRoomDefine.jsp"%>
	<%@ include file="vipRoomDataExt.jsp"%>
	<%@ include file="vipRoomFormExt.jsp"%>
	<%@ include file="vipRoomSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'system/vipRoom!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				...
				items : [
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/system/vipRoom!add">
						,getAddButton(win,winTitle + '——新增','新增'),
						getCopyAddButton(win,winTitle + '——拷贝添加', sm,'拷贝添加')
						</sec:authorize>
						<sec:authorize url="/system/vipRoom!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改'),
						getBatchButton(batchWin,sm,'批量修改')
						</sec:authorize>
						<sec:authorize url="/system/vipRoom!del">
						,getDelButton(ds, sm, 'system/vipRoom!delete','删除')
						</sec:authorize>
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'system/vipRoom!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/system/vipRoom!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/system/vipRoom!edit">,editAction</sec:authorize>
						<sec:authorize url="/system/vipRoom!del">,delAction</sec:authorize>
					 ]
				});
