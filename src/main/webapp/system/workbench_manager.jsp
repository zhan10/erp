...
<head>
	...
	<%@ include file="workbenchDefine.jsp"%>
	<%@ include file="workbenchDataExt.jsp"%>
	<%@ include file="workbenchFormExt.jsp"%>
	<%@ include file="workbenchSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'system/workbench!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				...
				items : [
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/system/workbench!add">
						,getAddButton(win,winTitle + '——新增','新增'),
						getCopyAddButton(win,winTitle + '——拷贝添加', sm,'拷贝添加')
						</sec:authorize>
						<sec:authorize url="/system/workbench!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改'),
						getBatchButton(batchWin,sm,'批量修改')
						</sec:authorize>
						<sec:authorize url="/system/workbench!del">
						,getDelButton(ds, sm, 'system/workbench!delete','删除')
						</sec:authorize>
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'system/workbench!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/system/workbench!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/system/workbench!edit">,editAction</sec:authorize>
						<sec:authorize url="/system/workbench!del">,delAction</sec:authorize>
					 ]
				});
