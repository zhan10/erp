...
<head>
	...
	<%@ include file="usersDefine.jsp"%>
	<%@ include file="usersDataExt.jsp"%>
	<%@ include file="usersFormExt.jsp"%>
	<%@ include file="usersSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			win = getWindow(winTitle,winWidth,winHeight,200,100,form);
			batchWin=getWindow('批量修改',320,160,320,160,batchForm);
			ds = getDs(mainFields, 'system/users!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				...
				items : [
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/system/users!add">
						,getAddButton(win,winTitle + '——新增','新增')/* ,
						getCopyAddButton(win,winTitle + '——拷贝添加', sm,'拷贝添加') */
						</sec:authorize>
						<sec:authorize url="/system/users!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改')/* ,
						getBatchButton(batchWin,sm,'批量修改') */
						</sec:authorize>
						<sec:authorize url="/system/users!del">
						,getDelButton(ds, sm, 'system/users!delete','删除')
						</sec:authorize>
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'system/users!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/system/users!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/system/users!edit">,editAction</sec:authorize>
						<sec:authorize url="/system/users!del">,delAction</sec:authorize>
					 ]
				});
