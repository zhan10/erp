...
<head>
	...
	<%@ include file="vTechCardOrdersDefine.jsp"%>
	<%@ include file="vTechCardOrdersDataExt.jsp"%>
	<%@ include file="vTechCardOrdersFormExt.jsp"%>
	<%@ include file="vTechCardOrdersSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'produce/vTechCardOrders!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				...
				items : [
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/produce/techCardOrders!add">
						,getAddButton(win,winTitle + '——新增','新增'),
						getCopyAddButton(win,winTitle + '——拷贝添加', sm,'拷贝添加')
						</sec:authorize>
						<sec:authorize url="/produce/techCardOrders!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改'),
						getBatchButton(batchWin,sm,'批量修改')
						</sec:authorize>
						<sec:authorize url="/produce/techCardOrders!del">
						,getDelButton(ds, sm, 'produce/techCardOrders!delete','删除')
						</sec:authorize>
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'produce/techCardOrders!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/produce/techCardOrders!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/produce/techCardOrders!edit">,editAction</sec:authorize>
						<sec:authorize url="/produce/techCardOrders!del">,delAction</sec:authorize>
					 ]
				});
