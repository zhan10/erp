...
<head>
	...
	<%@ include file="workbenchBookingDefine.jsp"%>
	<%@ include file="workbenchBookingDataExt.jsp"%>
	<%@ include file="workbenchBookingFormExt.jsp"%>
	<%@ include file="workbenchBookingSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'produce/workbenchBooking!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				...
				items : [
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/produce/workbenchBooking!add">
						,getAddButton(win,winTitle + '——新增','新增'),
						getCopyAddButton(win,winTitle + '——拷贝添加', sm,'拷贝添加')
						</sec:authorize>
						<sec:authorize url="/produce/workbenchBooking!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改'),
						getBatchButton(batchWin,sm,'批量修改')
						</sec:authorize>
						<sec:authorize url="/produce/workbenchBooking!del">
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'produce/workbenchBooking!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/produce/workbenchBooking!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/produce/workbenchBooking!edit">,editAction</sec:authorize>
						<sec:authorize url="/produce/workbenchBooking!del">,delAction</sec:authorize>
					 ]
				});
