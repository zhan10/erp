...
<script>
	var vipRoom_Fields = [
		...
		,{text : '<s:text name="vBookingVipRoom.code" />',dataIndex:'code',name:'code',align : 'center',width:100}
		,{text : '<s:text name="vBookingVipRoom.vipRoomName" />',dataIndex:'vipRoomName',name:'vipRoomName',align : 'center',width:100}
		,{text : '<s:text name="vBookingVipRoom.dateFrom" />',dataIndex:'dateFrom',name:'dateFrom',align : 'center',width:100,xtype: 'datecolumn', convert: convertDate}
	...
	var vipRoom_ds = getDs1(vipRoom_Fields, 'produce/vBookingVipRoom!managerExt','','order by code',5);
	vipRoom_ds.pageSize=999;
	
	var vipRoom_tbar = Ext.create('Ext.toolbar.Toolbar',{
		padding : '5 0 5 10',
		items : [
	...
	var  vipRoom_grid= Ext.create('Ext.grid.Panel', {
		id : 'vipRoom_grid',
		store : vipRoom_ds,
		margin : "5",
		//tbar : orders_tbar,		
		columns :  vipRoom_Fields,
		columnLines : true,
		flex:1,
		hidden:true
	});
	var vipRoom_form = Ext.widget('form', {
		itemId : 'vipRoom_form',
		autoScroll : true,
		overflowX : 'scroll',
		overflowY : 'scroll',
	...
	var vipRoom_win;
	vipRoom_grid.on('itemdblclick', function(item_grid, rec) {
		//showWinByRec(vipRoom_win, winTitle+'——查看细项', rec);		
	});
	//vipRoom_win = getWindow('订单细项',500,600,500,600,vipRoom_form);
