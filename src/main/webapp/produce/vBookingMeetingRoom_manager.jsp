...
<%@ include file="vBookingMeetingRoomDefine.jsp"%>
<%@ include file="vBookingMeetingRoomDataExt.jsp"%>
<%@ include file="vBookingMeetingRoomFormExt.jsp"%>
<%@ include file="vBookingMeetingRoomSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'produce/vBookingMeetingRoom!managerExt', baseSql,order);
			ds.load();
				...
				items : [
					...
					'-',
					 Ext.create('Ext.form.DateField',{
                      ...
                      '-',
					getTextField('code', 100, '',0, '<s:text name="vBookingMeetingRoom.code" />'),
					'-',
					getTextField('cityName', 60, '',0, '<s:text name="vBookingMeetingRoom.cityName" />'),
					'-',
					getTextField('buildingName', 60, '',0, '<s:text name="vBookingMeetingRoom.buildingName" />'),
					'-',
					getTextField('meetingRoomName',60, '',0, '<s:text name="vBookingMeetingRoom.meetingRoomName" />'),
					'-',
					getTextField('consumerName', 60, '',0, '<s:text name="vBookingMeetingRoom.consumerName" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderBy',		
						width : 80,
						...
						editable : false,
						store : [
							['','选择排序' ],['code', '<s:text name="vBookingMeetingRoom.code" />']
							,['name', '<s:text name="vBookingMeetingRoom.meetingRoomName" />']
							,['consumerName', '<s:text name="vBookingMeetingRoom.consumerName" />']
							,['buildingName', '<s:text name="vBookingMeetingRoom.buildingName" />']
							,['dateFrom', '<s:text name="vBookingMeetingRoom.dateFrom" />']
							,['dateTo', '<s:text name="vBookingMeetingRoom.dateTo" />']
							,['dateTo', '<s:text name="vBookingMeetingRoom.total" />']
							,['dateTo', '<s:text name="vBookingMeetingRoom.balanceTime" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						handler : function() {						
							...
							try {
								...
								t_sql = tbar.getComponent('meetingRoomName').getValue();
								if (t_sql != "")
									sqlStr += " and meetingRoomName like '%"+ t_sql+ "%' ";
						...
						'->','-'
						<sec:authorize url="/produce/vBookingMeetingRoom!balance">
						...
						<sec:authorize url="/produce/vBookingMeetingRoom!nobalance">
						,Ext.create('Ext.Button', {
							text : '取消结算',
							tooltip : '取消订单结算',
						...
						<sec:authorize url="/produce/vBookingMeetingRoom!del">
						,getDelButton(ds, sm, 'produce/meetingRoomBooking!delete','删除')
						</sec:authorize>
