...
<%@ include file="vCultivateRoomBookingDefine.jsp"%>
<%@ include file="vCultivateRoomBookingDataExt.jsp"%>
<%@ include file="vCultivateRoomBookingFormExt.jsp"%>
<%@ include file="vCultivateRoomBookingSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'produce/vCultivateRoomBooking!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				...
				items : [
					...
					Ext.create('Ext.form.DateField',{
                      ...
                      '-',
					getTextField('code', 100, '',0, '<s:text name="vBookingVipRoom.code" />'),
					'-',
					getTextField('cityName', 60, '',0, '<s:text name="vBookingVipRoom.cityName" />'),
					'-',
					getTextField('buildingName', 60, '',0, '<s:text name="vBookingVipRoom.buildingName" />'),
					'-',
					getTextField('vipRoomName',60, '',0, '<s:text name="vBookingVipRoom.vipRoomName" />'),
					'-',
					getTextField('consumerName', 60, '',0, '<s:text name="vBookingVipRoom.consumerName" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderBy',		
						width : 80,
						...
						editable : false,
						store : [
							['','选择排序' ],['code', '<s:text name="vBookingVipRoom.code" />']
							,['name', '<s:text name="vBookingVipRoom.vipRoomName" />']
							,['consumerName', '<s:text name="vBookingVipRoom.consumerName" />']
							,['buildingName', '<s:text name="vBookingVipRoom.buildingName" />']
							,['dateFrom', '<s:text name="vBookingVipRoom.dateFrom" />']
							,['dateTo', '<s:text name="vBookingVipRoom.dateTo" />']
							,['dateTo', '<s:text name="vBookingVipRoom.total" />']
							,['dateTo', '<s:text name="vBookingVipRoom.balanceTime" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						handler : function() {						
							...
							try {
								...
								t_sql = tbar.getComponent('vipRoomName').getValue();
								if (t_sql != "")
									sqlStr += " and vipRoomName like '%"+ t_sql+ "%' ";
						...
						'->','-'
						<sec:authorize url="/produce/vCultivateRoomBooking!balance">
						...
						<sec:authorize url="/produce/vCultivateRoomBooking!nobalance">
						,Ext.create('Ext.Button', {
							text : '取消结算',
							tooltip : '取消订单结算',
						...
						<sec:authorize url="/produce/vCultivateRoomBooking!del">
						,getDelButton(ds, sm, 'produce/cultivateRoomBooking!delete','删除')
						</sec:authorize>
