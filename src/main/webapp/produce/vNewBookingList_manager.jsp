<%@ page contentType="text/html;charset=UTF-8" pageEncoding="utf-8"%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">
<html>
<head>
	<%@ include file="../jspComm/extHeader.jsp"%>
	<%@ include file="vNewBookingListDefine.jsp"%>
	<script type="text/javascript" src="ini/getNewBookingStatusDs.js"></script>
	<!-- 引入日期时间控件 -->
	<script type="text/javascript" src="dateTime/UX_TimePickerField.js"></script>
	<script type="text/javascript" src="dateTime/UX_DateTimePicker.js"></script>
	<script type="text/javascript" src="dateTime/UX_DateTimeMenu.js"></script>
	<script type="text/javascript" src="dateTime/UX_DateTimeField.js"></script>
	<%@ include file="vNewBookingListDataExt.jsp"%>
	<%@ include file="vNewBookingListFormExt.jsp"%>
	<%@ include file="vNewBookingListSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		Ext.QuickTips.init();
		Ext.require([ 'Ext.grid.*', 'Ext.data.*', 'Ext.util.*','Ext.state.*' ]);
		Ext.onReady(function() {
			Ext.QuickTips.init();
			Ext.Date.defaultFormat='Y-m-d';
			win = getWindow(winTitle,winWidth,winHeight,500,400,form);
			smsWin = getWindow(winTitle,winWidth,winHeight,500,350,sendSmsForm);
			ds = getDs(mainFields, 'produce/vNewBookingList!managerExt', baseSql,order);
			ds.load();
			sm = Ext.create('Ext.selection.CheckboxModel', {
				listeners : {
					selectionchange : function(sm, selections) {
						grid.down('#tbar_btn_delete').setDisabled(selections.length == 0);
						grid.down('#tbar_btn_edit').setDisabled(selections.length != 1);
					}
				}
			});
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'status',
						width : 100,
						displayField: 'text',
						valueField: 'value',
						value:-1,
						label : '<s:text name="vNewBookingList.status" />',
						mode : 'local',
						selectOnFocus : true,
						editable : false,
						store : bookingStatusDs
					}),
					'-',
					getTextField('code', 100, '',0, '<s:text name="vNewBookingList.code" />'),
					'-',
					getTextField('productName', 100, '',0, '<s:text name="vNewBookingList.productName" />'),
					'-',
					getTextField('consumerName', 100, '',0, '<s:text name="vNewBookingList.consumerName" />'),
					'-',
					getTextField('tel', 100, '',0, '<s:text name="vNewBookingList.tel" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderBy',		
						width : 80,
						value : '',
						mode : 'local',
						editable : false,
						store : [
							['','选择排序' ]
							,['status', '<s:text name="vNewBookingList.status" />']
							,['buildingName', '<s:text name="vNewBookingList.buildingName" />']
							,['total', '<s:text name="vNewBookingList.total" />']
							,['dateFrom', '<s:text name="vNewBookingList.dateFrom" />']
							,['dateTo', '<s:text name="vNewBookingList.dateTo" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
						width : 60,
						value : 'asc',
						hideLabel : true,
						mode : 'local',
						selectOnFocus : true,
						editable : false,
						store : [['asc','顺序' ],['desc','倒序' ]]
					}),
					{
						text : '查询',
						iconCls : 'search',
						handler : function() {
							sqlStr = "";
							t_sql = "";
							try {
								t_sql = tbar.getComponent('status').getValue();
								if (t_sql!="-1")
									sqlStr += " and status ="+ t_sql;
								t_sql = tbar.getComponent('code').getValue();
								if (t_sql != "")
									sqlStr += " and code like '%"+ t_sql+ "%' ";
								t_sql = tbar.getComponent('productName').getValue();
								if (t_sql != "")
									sqlStr += " and productName like '%"+ t_sql+ "%' ";
								t_sql = tbar.getComponent('consumerName').getValue();
								if (t_sql != "")
									sqlStr += " and consumerName like '%"+ t_sql+ "%' ";
								t_sql = tbar.getComponent('tel').getValue();
								if (t_sql != "")
									sqlStr += " and tel like '%"+ t_sql+ "%' ";
								var orderBy = tbar.getComponent('orderBy').getValue();
								if (orderBy != ''){
									order = " order by "+ orderBy;
									var orderType = tbar.getComponent('orderType').getValue();
									if (orderType == 'desc')
										order += " desc";
								}
							}catch(e){
								Ext.MessageBox.alert('错误', '查询条件有误' + e);
							}
							Ext.apply(
								ds.proxy.extraParams,{whereSql : sqlStr,order : order});
								ds.loadPage(1);
							}
						},
						'->','-'
						<sec:authorize url="/produce/newBookingList!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改',function(){
							form.down('#status').setReadOnly(true);
							form.down('#code').setReadOnly(true);
							form.down('#productName').setReadOnly(true);
							form.down('#buildingName').setReadOnly(true);
							form.down('#consumerName').setReadOnly(true);
							form.down('#bookingTime').setReadOnly(true);
							form.down('#tel').setReadOnly(true);
							form.down('#email').setReadOnly(true);
							var type = form.down('#productType').getValue();
							form.down('#dateFrom1').setValue(form.down('#dateFrom').getValue());
							form.down('#dateTo1').setValue(form.down('#dateTo').getValue());
							if(type==3){
								form.down('#dateType1').hide();
								form.down('#dateType').show();
							}else{
								form.down('#dateType1').show();
								form.down('#dateType').hide();
							}
							
						})
						</sec:authorize>
						<sec:authorize url="/produce/newBookingList!del">
						,getDelButton(ds, sm, 'produce/newBooking!delete','删除')
						</sec:authorize>
					]
				});
				bbar = getBbar(ds);
				var showAction=Ext.create('Ext.Action', {
					iconCls: 'open',
					text: '查看',
					handler: function(widget, event) {
						showWin(win, winTitle+'——查看',sm);
						var type = form.down('#productType').getValue();
						form.down('#dateFrom1').setValue(form.down('#dateFrom').getValue());
						form.down('#dateTo1').setValue(form.down('#dateTo').getValue());
						if(type==3){
							form.down('#dateType1').hide();
							form.down('#dateType').show();
						}else{
							form.down('#dateType1').show();
							form.down('#dateType').hide();
						}
					}
				});
				var addAction = Ext.create('Ext.Action', {
					iconCls: 'add',
					text: '新增',
					handler: function(widget, event) {
						addWin(win, winTitle + '——新增');
					}
				});
				var copyAddAction = Ext.create('Ext.Action', {
					iconCls: 'copy',
					 text: '拷贝添加',
						handler: function(widget, event) { 
						copyAddWin(win,winTitle + '——拷贝添加', sm);   
					}
				});
				var editAction = Ext.create('Ext.Action', {
					iconCls: 'edit',
					 text: '修改',
						handler: function(widget, event) { 
						editWin(win,winTitle+'——修改', sm);
						form.down('#status').setReadOnly(true);
						form.down('#code').setReadOnly(true);
						form.down('#productName').setReadOnly(true);
						form.down('#buildingName').setReadOnly(true);
						form.down('#consumerName').setReadOnly(true);
						form.down('#bookingTime').setReadOnly(true);
						form.down('#tel').setReadOnly(true);
						form.down('#email').setReadOnly(true);
						var type = form.down('#productType').getValue();
						form.down('#dateFrom1').setValue(form.down('#dateFrom').getValue());
						form.down('#dateTo1').setValue(form.down('#dateTo').getValue());
						if(type==3){
							form.down('#dateType1').hide();
							form.down('#dateType').show();
						}else{
							form.down('#dateType1').show();
							form.down('#dateType').hide();
						}
					 }
				});
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'produce/newBooking!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/produce/newBookingList!edit">,editAction</sec:authorize>
						<sec:authorize url="/produce/newBookingList!del">,delAction</sec:authorize>
					 ]
				});
				grid = getGrid('grid',gridTitle,ds,mainColumns, sm, tbar, bbar);
				grid.on('itemdblclick', function(grid,rec) {
					showWinByRec(win,winTitle+'——查看',rec);
					var type = form.down('#productType').getValue();
					form.down('#dateFrom1').setValue(form.down('#dateFrom').getValue());
					form.down('#dateTo1').setValue(form.down('#dateTo').getValue());
					if(type==3){
						form.down('#dateType1').hide();
						form.down('#dateType').show();
					}else{
						form.down('#dateType1').show();
						form.down('#dateType').hide();
					}
				});
				grid.on('itemcontextmenu',function(view, rec, node, index, e) {
					e.stopEvent();
					contextMenu.showAt(e.getXY());
					return false;
				});
				var viewport = Ext.create('Ext.Viewport', {
							layout : 'fit',
							items : grid
					});	
		});
	</script>
</body>
</html>
