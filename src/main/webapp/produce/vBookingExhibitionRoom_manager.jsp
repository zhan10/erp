...
<%@ include file="vBookingExhibitionRoomDefine.jsp"%>
<%@ include file="vBookingExhibitionRoomDataExt.jsp"%>
<%@ include file="vBookingExhibitionRoomFormExt.jsp"%>
<%@ include file="vBookingExhibitionRoomSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'produce/vBookingExhibitionRoom!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				...
				items : [
					...
					Ext.create('Ext.form.DateField',{
                      ...
                      '-',
					getTextField('code', 100, '',0, '<s:text name="vBookingExhibitionRoom.code" />'),
					'-',
					getTextField('cityName', 60, '',0, '<s:text name="vBookingExhibitionRoom.cityName" />'),
					'-',
					getTextField('buildingName', 60, '',0, '<s:text name="vBookingExhibitionRoom.buildingName" />'),
					'-',
					getTextField('exhibitionRoomName',60, '',0, '<s:text name="vBookingExhibitionRoom.exhibitionRoomName" />'),
					'-',
					getTextField('consumerName', 60, '',0, '<s:text name="vBookingExhibitionRoom.consumerName" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderBy',		
						width : 80,
						...
						editable : false,
						store : [
							['','选择排序' ],['code', '<s:text name="vBookingExhibitionRoom.code" />']
							,['name', '<s:text name="vBookingExhibitionRoom.exhibitionRoomName" />']
							,['consumerName', '<s:text name="vBookingExhibitionRoom.consumerName" />']
							,['buildingName', '<s:text name="vBookingExhibitionRoom.buildingName" />']
							,['dateFrom', '<s:text name="vBookingExhibitionRoom.dateFrom" />']
							,['dateTo', '<s:text name="vBookingExhibitionRoom.dateTo" />']
							,['dateTo', '<s:text name="vBookingExhibitionRoom.total" />']
							,['dateTo', '<s:text name="vBookingExhibitionRoom.balanceTime" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						handler : function() {						
							...
							try {
								...
								t_sql = tbar.getComponent('exhibitionRoomName').getValue();
								if (t_sql != "")
									sqlStr += " and exhibitionRoomName like '%"+ t_sql+ "%' ";
						...
						'->','-'
						<sec:authorize url="/produce/vBookingExhibitionRoom!balance">
						...
						<sec:authorize url="/produce/vBookingExhibitionRoom!nobalance">
						,Ext.create('Ext.Button', {
							text : '取消结算',
							tooltip : '取消订单结算',
						...
						<sec:authorize url="/produce/vBookingExhibitionRoom!del">
						,getDelButton(ds, sm, 'produce/exhibitionRoomBooking!delete','删除')
						</sec:authorize>
