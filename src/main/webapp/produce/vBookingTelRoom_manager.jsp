...
<%@ include file="vBookingTelRoomDefine.jsp"%>
<%@ include file="vBookingTelRoomDataExt.jsp"%>
<%@ include file="vBookingTelRoomFormExt.jsp"%>
<%@ include file="vBookingTelRoomSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'produce/vBookingTelRoom!managerExt', baseSql,order);
			ds.load();
				...
				items : [
					...
					'-',
					 Ext.create('Ext.form.DateField',{
                      ...
                      '-',
					getTextField('code', 100, '',0, '<s:text name="vBookingTelRoom.code" />'),
					'-',
					getTextField('cityName', 60, '',0, '<s:text name="vBookingTelRoom.cityName" />'),
					'-',
					getTextField('buildingName', 60, '',0, '<s:text name="vBookingTelRoom.buildingName" />'),
					'-',
					getTextField('telRoomName',60, '',0, '<s:text name="vBookingTelRoom.telRoomName" />'),
					'-',
					getTextField('consumerName', 60, '',0, '<s:text name="vBookingTelRoom.consumerName" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderBy',		
						width : 80,
						...
						editable : false,
						store : [
							['','选择排序' ],['code', '<s:text name="vBookingTelRoom.code" />']
							,['name', '<s:text name="vBookingTelRoom.telRoomName" />']
							,['consumerName', '<s:text name="vBookingTelRoom.consumerName" />']
							,['buildingName', '<s:text name="vBookingTelRoom.buildingName" />']
							,['dateFrom', '<s:text name="vBookingTelRoom.dateFrom" />']
							,['dateTo', '<s:text name="vBookingTelRoom.dateTo" />']
							,['dateTo', '<s:text name="vBookingTelRoom.total" />']
							,['dateTo', '<s:text name="vBookingTelRoom.balanceTime" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						handler : function() {						
							...
							try {
								...
								t_sql = tbar.getComponent('telRoomName').getValue();
								if (t_sql != "")
									sqlStr += " and telRoomName like '%"+ t_sql+ "%' ";
						...
						'->','-'
						<sec:authorize url="/produce/vBookingTelRoom!balance">
						...
						<sec:authorize url="/produce/vBookingTelRoom!nobalance">
						,Ext.create('Ext.Button', {
							text : '取消结算',
							tooltip : '取消订单结算',
						...
						<sec:authorize url="/produce/vBookingTelRoom!del">
						,getDelButton(ds, sm, 'produce/telRoomBooking!delete','删除')
						</sec:authorize>
