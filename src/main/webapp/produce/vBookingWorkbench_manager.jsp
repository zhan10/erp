...
<%@ include file="vBookingWorkbenchDefine.jsp"%>
<%@ include file="vBookingWorkbenchDataExt.jsp"%>
<%@ include file="vBookingWorkbenchFormExt.jsp"%>
<%@ include file="vBookingWorkbenchSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'produce/vBookingWorkbench!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				...
				items : [
					...
					'-',
                      ...
                      '-',
					getTextField('code', 100, '',0, '<s:text name="vBookingWorkbench.code" />'),
					'-',
					getTextField('cityName', 60, '',0, '<s:text name="vBookingWorkbench.cityName" />'),
					'-',
					getTextField('buildingName', 60, '',0, '<s:text name="vBookingWorkbench.buildingName" />'),
					'-',
					getTextField('workbenchName',60, '',0, '<s:text name="vBookingWorkbench.workbenchName" />'),
					'-',
					getTextField('consumerName', 60, '',0, '<s:text name="vBookingWorkbench.consumerName" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderBy',		
						width : 80,
						...
						editable : false,
						store : [
							['','选择排序' ],['code', '<s:text name="vBookingWorkbench.code" />']
							,['name', '<s:text name="vBookingWorkbench.workbenchName" />']
							,['consumerName', '<s:text name="vBookingWorkbench.consumerName" />']
							,['buildingName', '<s:text name="vBookingWorkbench.buildingName" />']
							,['dateFrom', '<s:text name="vBookingWorkbench.dateFrom" />']
							,['dateTo', '<s:text name="vBookingWorkbench.dateTo" />']
							,['dateTo', '<s:text name="vBookingWorkbench.total" />']
							,['dateTo', '<s:text name="vBookingWorkbench.balanceTime" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						handler : function() {						
							...
							try {
								...
								t_sql = tbar.getComponent('workbenchName').getValue();
								if (t_sql != "")
									sqlStr += " and workbenchName like '%"+ t_sql+ "%' ";
						...
						'->','-'
						<sec:authorize url="/produce/vBookingWorkbench!balance">
						...
						<sec:authorize url="/produce/vBookingWorkbench!nobalance">
						,Ext.create('Ext.Button', {
							text : '取消结算',
							tooltip : '取消订单结算',
						...
						<sec:authorize url="/produce/vBookingWorkbench!del">
						,getDelButton(ds, sm, 'produce/workbenchBooking!delete','删除')
						</sec:authorize>
