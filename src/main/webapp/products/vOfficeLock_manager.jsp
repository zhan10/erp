...
<head>
	...
	<%@ include file="vOfficeLockDefine.jsp"%>
		<!-- 下拉框数据 -->
	<%@ include file="../c/cb_office.jsp"%>
	<!-- 引入日期时间控件 -->
	...
	<%@ include file="vOfficeLockDataExt.jsp"%>
	<%@ include file="vOfficeLockFormExt.jsp"%>
	<%@ include file="vOfficeLockSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'products/vOfficeLock!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('buildingName', 100, '',0, '<s:text name="vOfficeLock.buildingName" />'),
					'-',
					getTextField('floorName', 100, '',0, '<s:text name="vOfficeLock.floorName" />'),
					'-',
					getTextField('productName', 100, '',0, '<s:text name="vOfficeLock.productName" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['productName', '<s:text name="vOfficeLock.productName" />']
							,['lockFrom', '<s:text name="vOfficeLock.lockFrom" />']
							,['lockTo', '<s:text name="vOfficeLock.lockTo" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/products/officeLock!add">
						,getAddButton(win,winTitle + '——新增','新增')
						</sec:authorize>
						<sec:authorize url="/products/officeLock!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改')
						</sec:authorize>
						<sec:authorize url="/products/officeLock!del">
						,getDelButton(ds, sm, 'products/officeLock!delete','删除')
						</sec:authorize>
					]
				});
				bbar = getBbar(ds);
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'products/officeLock!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/products/officeLock!add">,addAction</sec:authorize>
						<sec:authorize url="/products/officeLock!edit">,editAction</sec:authorize>
						<sec:authorize url="/products/officeLock!del">,delAction</sec:authorize>
					 ]
				});
