...
<head>
	...
	<%@ include file="vWorkbenchLockDefine.jsp"%>
	<!-- 下拉框数据 -->
	<%@ include file="../c/cb_workbench.jsp"%>
	<!-- 引入日期时间控件 -->
	...
	<%@ include file="vWorkbenchLockDataExt.jsp"%>
	<%@ include file="vWorkbenchLockFormExt.jsp"%>
	<%@ include file="vWorkbenchLockSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'products/vWorkbenchLock!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('productName', 100, '',0, '<s:text name="vWorkbenchLock.productName" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['productName', '<s:text name="vWorkbenchLock.productName" />']
							,['quantity', '<s:text name="vWorkbenchLock.quantity" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/products/workbenchLock!add">
						,getAddButton(win,winTitle + '——新增','新增')
						</sec:authorize>
						<sec:authorize url="/products/workbenchLock!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改')
						</sec:authorize>
						<sec:authorize url="/products/workbenchLock!del">
						,getDelButton(ds, sm, 'products/workbenchLock!delete','删除')
						</sec:authorize>
					]
				});
				bbar = getBbar(ds);
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'products/workbenchLock!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/products/workbenchLock!add">,addAction</sec:authorize>
						<sec:authorize url="/products/workbenchLock!edit">,editAction</sec:authorize>
						<sec:authorize url="/products/workbenchLock!del">,delAction</sec:authorize>
					 ]
				});
