...
<head>
	...
	<%@ include file="vNewMeetingRoomDefine.jsp"%>
	<!-- 下拉框数据 -->
	<%@ include file="../c/cb_buildings.jsp"%>
	<%@ include file="../c/cb_floors.jsp"%>
	<%@ include file="vNewMeetingRoomDataExt.jsp"%>
	<%@ include file="vNewMeetingRoomFormExt.jsp"%>
	<%@ include file="vNewMeetingRoomBookingGridExt.jsp"%>
	<%@ include file="vNewMeetingRoomSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			bookingWin = getWindow(winTitle,winWidth,winHeight,500,200,meetingRoom_grid);
			ds = getDs(mainFields, 'products/vNewMeetingRoom!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('buildingName', 100, '',0, '<s:text name="vNewMeetingRoom.buildingName" />'),
					'-',
					getTextField('floorName', 100, '',0, '<s:text name="vNewMeetingRoom.floorName" />'),
					'-',
					getTextField('name', 100, '',0, '<s:text name="vNewMeetingRoom.name" />'),
					'-',
					getTextField('numbering', 100, '',0, '<s:text name="vNewMeetingRoom.numbering" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['buildingName', '<s:text name="vNewMeetingRoom.buildingName" />']
							,['floorName', '<s:text name="vNewMeetingRoom.floorName" />']
							,['name', '<s:text name="vNewMeetingRoom.name" />']
							,['numbering', '<s:text name="vNewMeetingRoom.numbering" />']
							,['workbenchNumber', '<s:text name="vNewMeetingRoom.workbenchNumber" />']
							,['price', '<s:text name="vNewMeetingRoom.price" />']
							,['deposit', '<s:text name="vNewMeetingRoom.deposit" />']
							,['payRatio', '<s:text name="vNewMeetingRoom.payRatio" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/products/newMeetingRoom!add">
						,getAddButton(win,winTitle + '——新增','新增')
						</sec:authorize>
						<sec:authorize url="/products/newMeetingRoom!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改',function(){
							...
							if(rec.get('planePic')!=''&&rec.get('planePic')!=null){
								form.down('#fileShow1').setValue('<a target="_blank" href="pics/newMeetingRoom/'+rec.get('planePic')+'">'+rec.get('planePic')+'</a>');//.setVisible(true);
							}else{
								form.down('#fileShow1').setValue('');
							}
							if(rec.get('coverPic')!=''&&rec.get('coverPic')!=null){
								form.down('#fileShow2').setValue('<a target="_blank" href="pics/newMeetingRoom/'+rec.get('coverPic')+'">'+rec.get('coverPic')+'</a>');//.setVisible(true);
							}else{
						...
						<sec:authorize url="/products/newMeetingRoom!del">
						,getDelButton(ds, sm, 'products/newMeetingRoom!delete','删除')
						</sec:authorize>
					]
				});
				bbar = getBbar(ds);
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'products/newMeetingRoom!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/products/newMeetingRoom!add">,addAction</sec:authorize>
						<sec:authorize url="/products/newMeetingRoom!edit">,editAction</sec:authorize>
						<sec:authorize url="/products/newMeetingRoom!del">,delAction</sec:authorize>
					 ]
				});
