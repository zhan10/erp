...
<head>
	...
	<%@ include file="vMeetingRoomLockDefine.jsp"%>
		<!-- 下拉框数据 -->
	<%@ include file="../c/cb_meetingRoom.jsp"%>
	<!-- 引入日期时间控件 -->
	...
	<%@ include file="vMeetingRoomLockDataExt.jsp"%>
	<%@ include file="vMeetingRoomLockFormExt.jsp"%>
	<%@ include file="vMeetingRoomLockSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			win = getWindow(winTitle,winWidth,winHeight,500,200,form);
			ds = getDs(mainFields, 'products/vMeetingRoomLock!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('buildingName', 100, '',0, '<s:text name="vMeetingRoomLock.buildingName" />'),
					'-',
					getTextField('floorName', 100, '',0, '<s:text name="vMeetingRoomLock.floorName" />'),
					'-',
					getTextField('productName', 100, '',0, '<s:text name="vMeetingRoomLock.productName" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['productName', '<s:text name="vMeetingRoomLock.productName" />']
							,['lockFrom', '<s:text name="vMeetingRoomLock.lockFrom" />']
							,['lockTo', '<s:text name="vMeetingRoomLock.lockTo" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/products/meetingRoomLock!add">
						,getAddButton(win,winTitle + '——新增','新增')
						</sec:authorize>
						<sec:authorize url="/products/meetingRoomLock!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改')
						</sec:authorize>
						<sec:authorize url="/products/meetingRoomLock!del">
						,getDelButton(ds, sm, 'products/meetingRoomLock!delete','删除')
						</sec:authorize>
					]
				});
				bbar = getBbar(ds);
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'products/meetingRoomLock!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/products/meetingRoomLock!add">,addAction</sec:authorize>
						<sec:authorize url="/products/meetingRoomLock!edit">,editAction</sec:authorize>
						<sec:authorize url="/products/meetingRoomLock!del">,delAction</sec:authorize>
					 ]
				});
