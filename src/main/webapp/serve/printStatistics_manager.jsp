...
<head>
	...
	<%@ include file="printStatisticsDefine.jsp"%>
	<%@ include file="printStatisticsDataExt.jsp"%>
	<%@ include file="printStatisticsFormExt.jsp"%>
	<%@ include file="printStatisticsSearchExt.jsp"%>
...
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			win = getWindow(winTitle,winWidth,winHeight,600,250,form);
			batchWin=getWindow('批量修改',320,160,320,160,batchForm);
			ds = getDs(mainFields, 'serve/printStatistics!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				...
				items : [
					Ext.create('Ext.form.DateField',{
		                  ...
		                  width : 100,
		                  format : 'Y-m-d',
		                  emptyText:'<s:text name="printStatistics.printTime" />'
	                }),
	                '-',
	                getTextField('printUser', 100, '',0, '<s:text name="printStatistics.printUser" />'),
					...
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['printTime', '<s:text name="printStatistics.printTime" />']
							,['printUser', '<s:text name="printStatistics.printUser" />']
							,['cardNumber', '<s:text name="vPrintRecord.cardNumber" />']
							,['printFormat', '<s:text name="printStatistics.printFormat" />']
							,['paperFormat', '<s:text name="printStatistics.paperFormat" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						handler : function() {
							...
							try {
								...
								t_sql = tbar.getComponent('printUser').getValue();
								if (t_sql != "")
									sqlStr += " and printUser like '%"+ t_sql+ "%' ";
								t_sql = tbar.getComponent('cardNumber').getValue();
								if (t_sql != "")
									sqlStr += " and cardNumber like '%"+ t_sql+ "%' ";
								t_sql=Ext.Date.format(Ext.Date.add(new Date(tbar.getComponent('printTime').getValue())),'Y-m-d');
								if (t_sql != ""&&t_sql!="1970-01-01")
									sqlStr+="and datediff(day,'"+t_sql+"',printTime)=0";
						...
						'->','-'
						<sec:authorize url="/serve/printStatistics!add">
						/* ,getAddButton(win,winTitle + '——新增','新增'), */
						/* getCopyAddButton(win,winTitle + '——拷贝添加', sm,'拷贝添加') */
						</sec:authorize>
						<sec:authorize url="/serve/printStatistics!edit">
						/* ,getEditButton(win,winTitle+'——修改', sm,'修改') */
						
						</sec:authorize>
						<sec:authorize url="/serve/printStatistics!del">
						/* ,getDelButton(ds, sm, 'serve/printStatistics!delete','删除') */
						</sec:authorize>
			...
			/* 	var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'serve/printStatistics!delete',function(){});
					}
				}); */
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						/* <sec:authorize url="/serve/printStatistics!add">,addAction</sec:authorize> */
						/* <sec:authorize url="/serve/printStatistics!edit">,editAction</sec:authorize> */
						/* <sec:authorize url="/serve/printStatistics!del">,delAction</sec:authorize> */
					 ]
				});
