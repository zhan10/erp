...
<head>
	...
	<%@ include file="vOrdersListDefine.jsp"%>
	<%@ include file="vOrdersListDataExt.jsp"%>
	<%@ include file="vOrdersListFormExt.jsp"%>
	<%@ include file="vOrdersListSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'serve/vOrdersList!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				...
				items : [
                   Ext.create('Ext.form.field.ComboBox',{
                       ...
                       valueField: 'value',
                       value:-1,
                       label : '<s:text name="vOrdersList.status" />',
                       mode : 'local',
                       selectOnFocus : true,
                       editable : false,
                    ...
                    Ext.create('Ext.form.DateField',{
                      ...
                      '-',
                    getTextField('consumerName', 100, '',0, '<s:text name="vOrdersList.consumerName" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderBy',		
						width : 80,
						...
						store : [
							['','选择排序' ]
							,['consumerName', '<s:text name="vOrdersList.consumerName" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						/*<sec:authorize url="/serve/ordersList!add">
						,getAddButton(win,winTitle + '——新增','新增'),
						getCopyAddButton(win,winTitle + '——拷贝添加', sm,'拷贝添加')
						</sec:authorize>*/
						/*<sec:authorize url="/serve/ordersList!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改')
						//getBatchButton(batchWin,sm,'批量修改')
						</sec:authorize>*/
						<sec:authorize url="/serve/vOrdersList!del">
						,getDelButton(ds, sm, 'serve/ordersList!delete','删除')
						</sec:authorize>
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'serve/ordersList!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						//<sec:authorize url="/serve/ordersList!add">,addAction,copyAddAction</sec:authorize>
						//<sec:authorize url="/serve/ordersList!edit">,editAction</sec:authorize>
						<sec:authorize url="/serve/vOrdersList!del">,delAction</sec:authorize>
					 ]
				});
