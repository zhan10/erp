...
<head>
	...
	<%@ include file="healthDefine.jsp"%>
	<%@ include file="healthDataExt.jsp"%>
	<%@ include file="healthFormExt.jsp"%>
	<%@ include file="healthSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('userId', 100, '',0, '<s:text name="health.userId"/>'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ],
							[ 'userId', '<s:text name="health.userId"/>'],
							[ 'checkedTime', '<s:text name="health.checkedTime"/>'],
							[ 'height', '<s:text name="health.height"/>'],
							[ 'weight', '<s:text name="health.weight"/>'],
							[ 'diastolicPressure', '<s:text name="health.diastolicPressure"/>'],
							[ 'systolicPressure', '<s:text name="health.systolicPressure"/>'],
							[ 'bodyFatRatio', '<s:text name="health.bodyFatRatio"/>'],
							[ 'oxygenSaturation', '<s:text name="health.oxygenSaturation"/>'],
							[ 'pulse', '<s:text name="health.pulse"/>']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						handler : function() {
							...
							t_sql = "";
							try {
								t_sql = tbar.getComponent('userId').getValue();
								if (t_sql != "")
									sqlStr += " and userId like '%"+ t_sql+ "%' ";
				...
				var contextMenu = Ext.create('Ext.menu.Menu', {
					...
					/* 	<sec:authorize url="/crm/health!add">,addAction,copyAddAction</sec:authorize> */
						<sec:authorize url="/crm/health!edit">,editAction</sec:authorize>
						<sec:authorize url="/crm/health!del">,delAction</sec:authorize>
					 ]
				});
