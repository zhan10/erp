...
<head>
	...
	<%@ include file="paymentTypeDefine.jsp"%>
	<%@ include file="paymentTypeDataExt.jsp"%>
	<%@ include file="paymentTypeFormExt.jsp"%>
	<%@ include file="paymentTypeSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'crm/paymentType!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('name', 100, '',0, '<s:text name="paymentType.name" />'),
					'-',
					getTextField('intro', 100, '',0, '<s:text name="paymentType.intro" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['name', '<s:text name="paymentType.name" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/crm/paymentType!add">
						,getAddButton(win,winTitle + '——新增','新增'),
						getCopyAddButton(win,winTitle + '——拷贝添加', sm,'拷贝添加')
						</sec:authorize>
						<sec:authorize url="/crm/paymentType!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改')/* ,
						getBatchButton(batchWin,sm,'批量修改') */
						</sec:authorize>
						<sec:authorize url="/crm/paymentType!del">
						,getDelButton(ds, sm, 'crm/paymentType!delete','删除')
						</sec:authorize>
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'crm/paymentType!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/crm/paymentType!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/crm/paymentType!edit">,editAction</sec:authorize>
						<sec:authorize url="/crm/paymentType!del">,delAction</sec:authorize>
					 ]
				});
