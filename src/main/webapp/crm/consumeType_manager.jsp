...
<head>
	...
	<%@ include file="consumeTypeDefine.jsp"%>
	<%@ include file="consumeTypeDataExt.jsp"%>
	<script type="text/javascript" src="ini/getChargeTypeDs.js"></script>
	<%@ include file="consumeTypeFormExt.jsp"%>
	<%@ include file="consumeTypeSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			win = getWindow(winTitle,winWidth,winHeight,500,280,form);
			batchWin=getWindow('批量修改',320,160,320,160,batchForm);
			ds = getDs(mainFields, 'crm/consumeType!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('code', 100, '',0, '<s:text name="consumeType.code" />'),
					'-',
					getTextField('name', 100, '',0, '<s:text name="consumeType.name" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['name', '<s:text name="consumeType.name" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/crm/consumeType!add">
						,getAddButton(win,winTitle + '——新增','新增'),
						getCopyAddButton(win,winTitle + '——拷贝添加', sm,'拷贝添加')
						</sec:authorize>
						<sec:authorize url="/crm/consumeType!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改')/* ,
						getBatchButton(batchWin,sm,'批量修改') */
						</sec:authorize>
						<sec:authorize url="/crm/consumeType!del">
						,getDelButton(ds, sm, 'crm/consumeType!delete','删除')
						</sec:authorize>
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'crm/consumeType!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/crm/consumeType!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/crm/consumeType!edit">,editAction</sec:authorize>
						<sec:authorize url="/crm/consumeType!del">,delAction</sec:authorize>
					 ]
				});
