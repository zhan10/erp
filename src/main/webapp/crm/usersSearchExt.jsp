...
<script>
	var rBrackets = [ ')', '' ];
	var searchColumns = [ [ 'id', '<s:text name="department.id" />' ],
			[ 'name', '<s:text name="department.name" />' ],
			[ 'linkman', '<s:text name="department.linkman" />' ],
			[ 'address', '<s:text name="department.address" />' ],
			[ 'tel', '<s:text name="department.tel" />' ],
			[ 'fax', '<s:text name="department.fax" />' ],
			[ 'email', '<s:text name="department.email" />' ],
			[ 'contactWay', '<s:text name="department.contactWay" />' ],
			[ 'introduction', '<s:text name="department.introduction" />' ],
			[ 'memo', '<s:text name="department.memo" />' ] ];
	var searchOperates = [ '=', '>', '>=', '<', '<=', '!=', 'like', 'not like',
			'in', 'not in' ];
	...
	for (i = 0; i < searchLine; i++) {
		...
		searchForm.add({
			...
			layout : 'hbox',
			margin : '5 5 5 5',
			items : [ lBrs[i], fieldName[i], operation[i], selectNameValue[i],
					rBrs[i], andOrs[i] ]
		});
	}
		...
		items : [
				{
					...
					layout : 'hbox',
					margin : '5 5 5 5',
					items : [ orderByField[0], orderType[0], orderByField[1],
							orderType[1] ]
				},
				{
					xtype : 'container',
					layout : 'hbox',
					margin : '5 5 5 5',
					items : [ orderByField[2], orderType[2], orderByField[3],
							orderType[3] ]
				} ]
	});
