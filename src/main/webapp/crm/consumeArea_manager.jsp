...
<head>
	...
	<%@ include file="consumeAreaDefine.jsp"%>
	<%@ include file="consumeAreaDataExt.jsp"%>
	<%@ include file="consumeAreaFormExt.jsp"%>
	<%@ include file="consumeAreaSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'crm/consumeArea!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('name', 100, '',0, '<s:text name="consumeArea.name" />'),
					'-',
					getTextField('intro', 100, '',0, '<s:text name="consumeArea.intro" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['name', '<s:text name="consumeArea.name" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/crm/consumeArea!add">
						,getAddButton(win,winTitle + '——新增','新增'),
						getCopyAddButton(win,winTitle + '——拷贝添加', sm,'拷贝添加')
						</sec:authorize>
						<sec:authorize url="/crm/consumeArea!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改')
						</sec:authorize>
						<sec:authorize url="/crm/consumeAreae!del">
						,getDelButton(ds, sm, 'crm/consumeArea!delete','删除')
						</sec:authorize>
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'crm/consumeArea!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/crm/consumeArea!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/crm/consumeArea!edit">,editAction</sec:authorize>
						<sec:authorize url="/crm/consumeArea!del">,delAction</sec:authorize>
					 ]
				});
