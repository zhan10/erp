...
<head>
	...
	<%@ include file="vFloorDefine.jsp"%>
	<%@ include file="vFloorDataExt.jsp"%>
		<%@ include file="../c/cb_buildings.jsp"%>
	<%@ include file="vFloorFormExt.jsp"%>
	<%@ include file="vFloorSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'product/vFloor!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('buildingName', 100, '',0, '<s:text name="vFloor.buildingName" />'),
					'-',
					getTextField('name', 100, '',0, '<s:text name="vFloor.name" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['cityName', '<s:text name="vFloor.buildingName" />']
							,['name', '<s:text name="vFloor.name" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/product/vFloor!add">
						...
						<sec:authorize url="/product/vFloor!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改',function(){
						var rec=(sm.getSelection())[0];
						if(rec.get('pic')!=''){
						form.down('#fileShow').setValue('<a target="_blank" href="pics/floor/'+rec.get('pic')+'">'+rec.get('pic')+'</a>');
					}else{
						...
						<sec:authorize url="/product/vFloor!del">
						,getDelButton(ds, sm, 'product/floor!delete','删除')
						</sec:authorize>
					]
				});
				bbar = getBbar(ds);
				var showAction=Ext.create('Ext.Action', {
					...
					handler: function(widget, event) {
						...
						var rec=(sm.getSelection())[0];
						if(rec.get('pic')!=''){
							form.down('#fileShow').setValue('<a target="_blank" href="pics/floor/'+rec.get('pic')+'">'+rec.get('pic')+'</a>');//.setVisible(true);
						}else{
							form.down('#fileShow').setValue('');
						}
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'product/floor!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/product/vFloor!add">,addAction</sec:authorize>
						<sec:authorize url="/product/vFloor!edit">,editAction</sec:authorize>
						<sec:authorize url="/product/vFloor!del">,delAction</sec:authorize>
					 ]
				});
