...
<head>
	...
	<%@ include file="vTelRoomDefine.jsp"%>
	<%@ include file="vTelRoomDataExt.jsp"%>
	...
	<%@ include file="vTelRoomFormExt.jsp"%>
	<%@ include file="vTelRoomSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'product/vTelRoom!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('cityName', 100, '',0, '<s:text name="vTelRoom.cityName" />'),
					'-',
					getTextField('areaName', 100, '',0, '<s:text name="vTelRoom.areaName" />'),
					'-',
					getTextField('buildingName', 100, '',0, '<s:text name="vTelRoom.buildingName" />'),
					'-',
					getTextField('name', 100, '',0, '<s:text name="vTelRoom.name" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['cityName', '<s:text name="vTelRoom.cityName" />']
							,['areaName', '<s:text name="vTelRoom.areaName" />']
							,['buildingName', '<s:text name="vTelRoom.buildingName" />']
							,['floorName', '<s:text name="vTelRoom.floorName" />']
							,['name', '<s:text name="vTelRoom.name" />']
							,['galleryful', '<s:text name="vTelRoom.galleryful" />']
							,['price', '<s:text name="vTelRoom.price" />']
							,['equipment', '<s:text name="vTelRoom.equipment" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/product/vTelRoom!add">
						...
						<sec:authorize url="/product/vTelRoom!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改',function(){
						var rec=(sm.getSelection())[0];
						setEquipment();
						if(rec.get('pic')!=''){
						form.down('#fileShow').setValue('<a target="_blank" href="pics/telRoom/'+rec.get('pic')+'">'+rec.get('pic')+'</a>');
					}else{
						...
						<sec:authorize url="/product/vTelRoom!del">
						,getDelButton(ds, sm, 'product/telRoom!delete','删除')
						</sec:authorize>
				...
				var showAction=Ext.create('Ext.Action', {
					...
					handler: function(widget, event) {
						...
						var rec=(sm.getSelection())[0];
						if(rec.get('pic')!=''){
							form.down('#fileShow').setValue('<a target="_blank" href="pics/telRoom/'+rec.get('pic')+'">'+rec.get('pic')+'</a>');//.setVisible(true);
						}else{
							form.down('#fileShow').setValue('');
						}
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'product/telRoom!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/product/vTelRoom!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/product/vTelRoom!edit">,editAction</sec:authorize>
						<sec:authorize url="/product/vTelRoom!del">,delAction</sec:authorize>
						<sec:authorize url="/product/vTelRoom!editPrice">,editPriceAction</sec:authorize>
					 ]
				});
