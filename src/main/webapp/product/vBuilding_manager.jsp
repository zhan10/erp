...
<head>
	...
	<%@ include file="vBuildingDefine.jsp"%>
	<%@ include file="vBuildingDataExt.jsp"%>
		<%@ include file="../c/cb_citys.jsp"%>
		<%@ include file="../c/cb_areas.jsp"%>
		<%@ include file="../c/cb_circles.jsp"%>
	<%@ include file="vBuildingFormExt.jsp"%>
	<%@ include file="vBuildingSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'product/vBuilding!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('cityName', 100, '',0, '<s:text name="vBuilding.cityName" />'),
					'-',
					getTextField('areaName', 100, '',0, '<s:text name="vBuilding.areaName" />'),
					'-',
					getTextField('name', 100, '',0, '<s:text name="vBuilding.name" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['cityName', '<s:text name="vBuilding.cityName" />']
							,['areaName', '<s:text name="vBuilding.areaName" />']
							,['name', '<s:text name="vBuilding.name" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/product/vBuilding!add">
						...
						<sec:authorize url="/product/vBuilding!edit">
						...
						<sec:authorize url="/product/vBuilding!del">
						,getDelButton(ds, sm, 'product/building!delete','删除')
						</sec:authorize>
					]
				});
				bbar = getBbar(ds);
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'product/building!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/product/vBuilding!add">,addAction</sec:authorize>
						<sec:authorize url="/product/vBuilding!edit">,editAction</sec:authorize>
						<sec:authorize url="/product/vBuilding!del">,delAction</sec:authorize>
					 ]
				});
