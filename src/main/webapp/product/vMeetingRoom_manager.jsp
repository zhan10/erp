...
<head>
	...
	<script type="text/javascript" src="ini/getDatesDs.js"></script>
	<%@ include file="vMeetingRoomDefine.jsp"%>
	<%@ include file="vMeetingRoomDataExt.jsp"%>
	...
	<%@ include file="vMeetingRoomFormExt.jsp"%>
	<%@ include file="vMeetingRoomSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'product/vMeetingRoom!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('cityName', 100, '',0, '<s:text name="vMeetingRoom.cityName" />'),
					'-',
					getTextField('areaName', 100, '',0, '<s:text name="vMeetingRoom.areaName" />'),
					'-',
					getTextField('buildingName', 100, '',0, '<s:text name="vMeetingRoom.buildingName" />'),
					'-',
					getTextField('name', 100, '',0, '<s:text name="vMeetingRoom.name" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['cityName', '<s:text name="vMeetingRoom.cityName" />']
							,['areaName', '<s:text name="vMeetingRoom.areaName" />']
							,['buildingName', '<s:text name="vMeetingRoom.buildingName" />']
							,['floorName', '<s:text name="vMeetingRoom.floorName" />']
							,['name', '<s:text name="vMeetingRoom.name" />']
							,['galleryful', '<s:text name="vMeetingRoom.galleryful" />']
							,['price', '<s:text name="vMeetingRoom.price" />']
							,['equipment', '<s:text name="vMeetingRoom.equipment" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/product/vMeetingRoom!add">
						...
						<sec:authorize url="/product/vMeetingRoom!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改',function(){
						var rec=(sm.getSelection())[0];
						setEquipment();
						if(rec.get('pic')!=''){
						form.down('#fileShow').setValue('<a target="_blank" href="pics/meetingRoom/'+rec.get('pic')+'">'+rec.get('pic')+'</a>');
					}else{
						...
						<sec:authorize url="/product/vMeetingRoom!del">
						,getDelButton(ds, sm, 'product/meetingRoom!delete','删除')
						</sec:authorize>
				...
				var showAction=Ext.create('Ext.Action', {
					...
					handler: function(widget, event) {
						...
						var rec=(sm.getSelection())[0];
						if(rec.get('pic')!=''){
							form.down('#fileShow').setValue('<a target="_blank" href="pics/meetingRoom/'+rec.get('pic')+'">'+rec.get('pic')+'</a>');//.setVisible(true);
						}else{
							form.down('#fileShow').setValue('');
						};
						if(rec.get('bigPic')!=''){
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'product/meetingRoom!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/product/vMeetingRoom!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/product/vMeetingRoom!edit">,editAction</sec:authorize>
						<sec:authorize url="/product/vMeetingRoom!del">,delAction</sec:authorize>
						<sec:authorize url="/product/vMeetingRoom!editPrice">,editPriceAction</sec:authorize>
