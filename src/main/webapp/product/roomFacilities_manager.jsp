...
<head>
	...
	<%@ include file="roomFacilitiesDefine.jsp"%>
	<%@ include file="roomFacilitiesDataExt.jsp"%>
	<%@ include file="roomFacilitiesFormExt.jsp"%>
	<%@ include file="roomFacilitiesSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			//获取数据
			ds = getDs(mainFields, 'product/roomFacilities!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('facilities', 100, '',0, '<s:text name="roomFacilities.facilities" />'),
					'-',	
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['facilities', '<s:text name="roomFacilities.facilities" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						handler : function() {
							...
							t_sql = "";
							try {
								t_sql = tbar.getComponent('facilities').getValue();
								if (t_sql != "")
									sqlStr += " and facilities like '%"+ t_sql+ "%' ";
						...
						'->','-'
						<sec:authorize url="/product/roomFacilities!add">
						...
						<sec:authorize url="/product/roomFacilities!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改',function(){
						var rec=(sm.getSelection())[0];
						if(rec.get('pic')!=''){
						form.down('#fileShow').setValue('<a target="_blank" href="pics/roomFacilities/'+rec.get('pic')+'">'+rec.get('pic')+'</a>');
					}else{
						...
						<sec:authorize url="/product/roomFacilities!del">
						,getDelButton(ds, sm, 'product/roomFacilities!delete','删除')
						</sec:authorize>
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'product/roomFacilities!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/product/roomFacilities!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/product/roomFacilities!edit">,editAction</sec:authorize>
						<sec:authorize url="/product/roomFacilities!del">,delAction</sec:authorize>
					 ]
				});
