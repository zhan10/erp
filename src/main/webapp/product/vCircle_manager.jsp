...
<head>
	...
	<%@ include file="vCircleDefine.jsp"%>
	<%@ include file="vCircleDataExt.jsp"%>
		<%@ include file="../c/cb_citys.jsp"%>
	<%@ include file="vCircleFormExt.jsp"%>
	<%@ include file="vCircleSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'product/vCircle!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('cityName', 100, '',0, '<s:text name="vCircle.cityName" />'),
					'-',
					getTextField('name', 100, '',0, '<s:text name="vCircle.name" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['cityName', '<s:text name="vCircle.cityName" />']
							,['name', '<s:text name="vCircle.name" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/product/vCircle!add">
						...
						<sec:authorize url="/product/vCircle!edit">
						...
						<sec:authorize url="/product/vCircle!del">
						,getDelButton(ds, sm, 'product/circle!delete','删除')
						</sec:authorize>
				...
				var showAction=Ext.create('Ext.Action', {
					...
					handler: function(widget, event) {
						...
						var rec=(sm.getSelection())[0];
						if(rec.get('pic')!=''){
							form.down('#fileShow').setValue('<a target="_blank" href="pics/circle/'+rec.get('pic')+'">'+rec.get('pic')+'</a>');//.setVisible(true);
						}else{
							form.down('#fileShow').setValue('');
						}
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'product/circle!delete',function(){});
					}
				});
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/product/vCircle!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/product/vCircle!edit">,editAction</sec:authorize>
						<sec:authorize url="/product/vCircle!del">,delAction</sec:authorize>
					 ]
				});
