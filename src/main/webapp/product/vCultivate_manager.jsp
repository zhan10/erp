...
<head>
	...
	<%@ include file="vCultivateDefine.jsp"%>
	<%@ include file="vCultivateDataExt.jsp"%>
	...
	<%@ include file="vCultivateFormExt.jsp"%>
	<%@ include file="vCultivateSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'product/vCultivateRoom!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('cityName', 100, '',0, '<s:text name="vCultivateManager.cityName" />'),
					'-',
					getTextField('areaName', 100, '',0, '<s:text name="vCultivateManager.areaName" />'),
					'-',
					getTextField('buildingName', 100, '',0, '<s:text name="vCultivateManager.buildingName" />'),
					'-',
					getTextField('name', 100, '',0, '<s:text name="vCultivateManager.name" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['cityName', '<s:text name="vCultivateManager.cityName" />']
							,['areaName', '<s:text name="vCultivateManager.areaName" />']
							,['buildingName', '<s:text name="vCultivateManager.buildingName" />']
							,['floorName', '<s:text name="vCultivateManager.floorName" />']
							,['name', '<s:text name="vCultivateManager.name" />']
							,['galleryful', '<s:text name="vCultivateManager.galleryful" />']
							,['price', '<s:text name="vCultivateManager.price" />']
							,['equipment', '<s:text name="vCultivateManager.equipment" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/product/vCultivateRoom!add">
						...
						<sec:authorize url="/product/vCultivateRoom!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改',function(){
						setEquipment();
						var rec=(sm.getSelection())[0];
						if(rec.get('pic')!=''){
						form.down('#fileShow').setValue('<a target="_blank" href="pics/cultivateRoom/'+rec.get('pic')+'">'+rec.get('pic')+'</a>');
					}else{
						...
						<sec:authorize url="/product/vTelRoom!editPrice">
						,getEditPriceButton(winPrice,winTitle+'——改价', sm,'改价',function(){
							formEditPrice.down('#name').setReadOnly(true);
						})
						</sec:authorize>
						<sec:authorize url="/product/vCultivateRoom!del">
						,getDelButton(ds, sm, 'product/cultivateRoom!delete','删除')
						</sec:authorize>
				...
				var showAction=Ext.create('Ext.Action', {
					...
					handler: function(widget, event) {
						...
						var rec=(sm.getSelection())[0];
						if(rec.get('pic')!=''){
							form.down('#fileShow').setValue('<a target="_blank" href="pics/cultivateRoom/'+rec.get('pic')+'">'+rec.get('pic')+'</a>');//.setVisible(true);
						}else{
							form.down('#fileShow').setValue('');
						}
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'product/cultivateRoom!delete',function(){});
					}
				});
				...
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/product/vCultivateRoom!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/product/vCultivateRoom!edit">,editAction</sec:authorize>
						<sec:authorize url="/product/vCultivateRoom!del">,delAction</sec:authorize>
						<sec:authorize url="/product/vCultivateRoom!editPrice">,editPriceAction</sec:authorize>
					]
				});
