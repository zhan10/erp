...
<head>
	...
	<%@ include file="vExhibitionRoomDefine.jsp"%>
	<%@ include file="vExhibitionRoomDataExt.jsp"%>
	...
	<%@ include file="vExhibitionRoomFormExt.jsp"%>
	<%@ include file="vExhibitionRoomSearchExt.jsp"%>
</head>
<body>
	<script language="javascript">
		...
		Ext.onReady(function() {
			...
			ds = getDs(mainFields, 'product/vExhibitionRoom!managerExt', baseSql,order);
			ds.load();
			...
			tbar = Ext.create('Ext.toolbar.Toolbar',{
				padding : '5 0 5 10',
				items : [
					getTextField('cityName', 100, '',0, '<s:text name="vExhibitionRoom.cityName" />'),
					'-',
					getTextField('areaName', 100, '',0, '<s:text name="vExhibitionRoom.areaName" />'),
					'-',
					getTextField('buildingName', 100, '',0, '<s:text name="vExhibitionRoom.buildingName" />'),
					'-',
					getTextField('name', 100, '',0, '<s:text name="vExhibitionRoom.name" />'),
					'-',
					Ext.create('Ext.form.field.ComboBox',{
						...
						store : [
							['','选择排序' ]
							,['cityName', '<s:text name="vExhibitionRoom.cityName" />']
							,['areaName', '<s:text name="vExhibitionRoom.areaName" />']
							,['buildingName', '<s:text name="vExhibitionRoom.buildingName" />']
							,['floorName', '<s:text name="vExhibitionRoom.floorName" />']
							,['name', '<s:text name="vExhibitionRoom.name" />']
							,['galleryful', '<s:text name="vExhibitionRoom.galleryful" />']
							,['price', '<s:text name="vExhibitionRoom.price" />']
							,['equipment', '<s:text name="vExhibitionRoom.equipment" />']
						]
					}),
					Ext.create('Ext.form.field.ComboBox',{
						itemId : 'orderType',
					...
					}),
					{
						...
						'->','-'
						<sec:authorize url="/product/vExhibitionRoom!add">
						...
						<sec:authorize url="/product/vExhibitionRoom!edit">
						,getEditButton(win,winTitle+'——修改', sm,'修改',function(){
						var rec=(sm.getSelection())[0];
						setEquipment();
						if(rec.get('pic')!=''){
						form.down('#fileShow').setValue('<a target="_blank" href="pics/exhibitionRoom/'+rec.get('pic')+'">'+rec.get('pic')+'</a>');
					}else{
						...
						<sec:authorize url="/product/vExhibitionRoom!del">
						,getDelButton(ds, sm, 'product/exhibitionRoom!delete','删除')
						</sec:authorize>
				...
				var delAction = Ext.create('Ext.Action', {
					iconCls: 'delete',
					 text: '删除',
						handler: function(widget, event) {   
						delFromDB(ds,sm,'product/exhibitionRoom!delete',function(){});
					}
				});
				...
				var contextMenu = Ext.create('Ext.menu.Menu', {
					 items: [
						showAction,'-'
						<sec:authorize url="/product/vExhibitionRoom!add">,addAction,copyAddAction</sec:authorize>
						<sec:authorize url="/product/vExhibitionRoom!edit">,editAction</sec:authorize>
						<sec:authorize url="/product/vExhibitionRoom!del">,delAction</sec:authorize>
						<sec:authorize url="/product/vExhibitionRoom!editPrice">,editPriceAction</sec:authorize>
					 ]
				});
